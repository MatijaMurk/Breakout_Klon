...
TTF_Font* AssetManager::GetFont(std::string filename, int size) {
	...
	if (mFonts[key] == nullptr) {
		...
		if(mFonts[key]==nullptr){
			printf("Font load error: Font(%s) Error(%s)", filename.c_str(), TTF_GetError());
	}
}
	return mFonts[key];
...
Mix_Music* AssetManager::GetMusic(std::string filename) {
	...
	if (mMusic[fullPath] == nullptr) {
		...
		if (mMusic[fullPath] == NULL) {
			printf("Music load error: File(%s) Error(%s)", filename.c_str(), Mix_GetError());
		}
	}
...
Mix_Chunk* AssetManager::GetChunk(std::string filename) {
	...
	if (mSFX[fullPath] == nullptr) {
		...
		if (mSFX[fullPath] == NULL) {
			printf("Chunk load error: File(%s) Error(%s)", filename.c_str(), Mix_GetError());
		}
	}
